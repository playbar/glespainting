...
bool Renderer::init() {
    ...
    mProgramBlurRow = createProgram(vertexShaderprocessRow, fragmentShaderprocessRow);
    if( mProgramBlurRow == 0 )
        return false;

    mProgramBlurCol = createProgram(vertexShaderprocessCol, fragmentShaderprocessCol);
    if( mProgramBlurCol == 0 )
        return false;
    ...
    //mFboRow
    glGenFramebuffers(1, &mFboRow);
    glBindFramebuffer(GL_FRAMEBUFFER, mFboRow);
    {
        glGenTextures(1, &mTexRow);
        glBindTexture(GL_TEXTURE_2D, mTexRow);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, _fbo_width, _fbo_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, mTexRow, 0);
    ...
    //mFboCol
    glGenFramebuffers(1, &mFboCol);
    glBindFramebuffer(GL_FRAMEBUFFER, mFboCol);
    {
        glGenTextures(1, &mTexCol);
        glBindTexture(GL_TEXTURE_2D, mTexCol);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, _fbo_width, _fbo_height, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, mTexCol, 0);
...
void Renderer::drawRow()
{
    ...
    glUseProgram(mProgramBlurRow);

    glBindBuffer(GL_ARRAY_BUFFER, mVBo0);
    GLint ptloc = glGetAttribLocation(mProgramBlurRow, "pos");
    GLint tcloc = glGetAttribLocation(mProgramBlurRow, "tcoord");
    glVertexAttribPointer(ptloc, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid *) offsetof(Vertex, x));
    glVertexAttribPointer(tcloc, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid *) offsetof(Vertex, u));
    glEnableVertexAttribArray(ptloc);
    glEnableVertexAttribArray(tcloc);
...

    {
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, mTexOrigin);
        loc = glGetUniformLocation(mProgramBlurRow, "texture");
        glUniform1i(loc, 0);
...
void Renderer::drawCol()
{
    ...
    glDisable(GL_DEPTH_TEST);
    glDisable(GL_STENCIL_TEST);
    glUseProgram(mProgramBlurCol);

    glBindBuffer(GL_ARRAY_BUFFER, mVBo0);
    GLint ptloc = glGetAttribLocation(mProgramBlurCol, "pos");
    GLint tcloc = glGetAttribLocation(mProgramBlurCol, "tcoord");
    glVertexAttribPointer(ptloc, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid *) offsetof(Vertex, x));
    glVertexAttribPointer(tcloc, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (const GLvoid *) offsetof(Vertex, u));
    glEnableVertexAttribArray(ptloc);
    glEnableVertexAttribArray(tcloc);
...

    {
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, mTexRow);
        loc = glGetUniformLocation(mProgramBlurCol, "texture");
        glUniform1i(loc, 0);
        glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
    }
